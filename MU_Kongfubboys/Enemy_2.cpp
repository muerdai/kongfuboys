...
* 攻击玩家
*/
void CEnemy_2::Attack()
{
	if (!m_keyword && !m_isboxing && !m_iskickaleg && !m_isshoot &&!m_downshoot)
	{
		...
		else if (m_isshoot)
		{
			...
			if (m_waittime == WAITTIME)
			{
				...
				if (m_waittime == TWELVE*TWO)
				{
					...
					m_boompoint.y =GOUNDY + THREE*TWELVE;
				}
			}
		}
		else if (m_downshoot)
...
*绘制图片
*/
void CEnemy_2::DrawBack(IplImage* bg)
{
	...
	if (m_boompoint.x <= ZERO || m_boompoint.x >= BGBOUNDARY)
	{
		m_isboom = false;
	}
	if (m_boompoint.x==m_enemypos.x)
		m_isboom=false;
	if (!m_deadtime)
	{
		...
		if (m_isboom)
		{
			CTools::PaintImg(m_img[ELEVEN], bg, m_boompoint, m_isturn);
		}
	}
	else 
		CTools::PaintImg(m_img[EIGHT], bg, m_playerpos, m_isturn);
